...
for train_ind, test_ind in kf.split(xs):
    ...
    hist = model.fit(train_xs, train_ys, epochs=30, batch_size=32, verbose=0).history
...
with open("model/overfit/confusion_matrix.npy", "wb") as handler:
    np.save(handler, np.array(conf_matrix).astype(int))

pd.DataFrame.from_dict(history).to_csv("model/overfit/history.csv", index=False)

stats = {
    "accuracy": results[1],
    "loss": results[0]
}
with open("model/overfit/results.json", "w") as handler:
