...
```{r}
history <- read_csv("model/overfit/history.csv")
```

```{r}
np <- import("numpy")

confusion_matrix <- np$load("model/overfit/confusion_matrix.npy")
...
results <- fromJSON(file="model/overfit/results.json")
...
```{r}
ggsave(filename="model/overfit/plots/loss.png", plot=loss)
ggsave(filename="model/overfit/plots/accuracy.png", plot=accuracy)
```
